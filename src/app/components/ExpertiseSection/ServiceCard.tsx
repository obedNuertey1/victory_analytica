...
export const ServiceCard: React.FC<ServiceCardProps> = ({
  ...
  return (
        <>
      <Link href={`/services/${id}`}  className="inline-block cursor-default not-sm:hidden shadow-lg hover:shadow-xl card-container rounded-4xl size-[12rem] md:size-[20rem] relative overflow-hidden">
        ...
        <motion.div  className="z-10 w-full h-[calc(20rem*2)] min-h-[auto] transform transition-all duration-700 ease-in-out scroll-auto hover:overflow-y-auto rounded-[inherit] hover:bg-gray-800 hover:text-white hover:translate-y-[-15rem] translate-y-[-0.5rem] scroll-m-0 scroll-p-0">
          <div className="w-full transform not-sm:translate-y-[6rem] sm:translate-y-[15rem] py-4 px-2 font-light z-10  mb-5 ">
            ...
            <span onClick={()=>{router.push(`/services/${id}`)}} className='group cursor-pointer mx-auto text-white bg-green-400 overflow-hidden hover:cursor-pointer p-3 rounded-lg font-bold  transition-all duration-300 hover:pr-4 hover:bg-green-400/80 inline-block'>
